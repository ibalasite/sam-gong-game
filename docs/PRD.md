...
Step 3: 閒家依順時鐘順序逐一決策
        ...
        → Call：下注與莊家等額
          【F15 Call預扣機制】Call確認後，Server立即預扣閒家籌碼：player chip_balance -= banker_bet_amount（與莊家Step 2 escrow機制對稱）；預扣後玩家進入等待翻牌階段；結算時：勝→退回預扣額+賠率；敗→預扣額歸底池
...
| 9 點 | 2 | 3× 下注額 | 2× 下注額 |
| 8 點以下（0–8 點，含8點，非三公）| 1 | 2× 下注額 | 1× 下注額 |
| 平手 | 0（退注）| 1× 下注額 | 0 |
| 閒家敗 | N/A | 0 | -1× 下注額 |
