...
| O3：變現 | ARPPU ≥ USD 10 | REQ-020a（Must Have）, REQ-020b（Should Have） | BDD S-020 | 🔲 待 PRD 填入 |
...
**莊家結算破產處理**：若莊家在結算過程中籌碼不足以支付所有贏家賠率，採用以下規則：(1) 依閒家**順時鐘**順序逐一結算；(2) 每位贏家依序收取其應得賠率（先到先得）；(3) 莊家籌碼歸零後，後續排隊贏家所得為零（不按比例分配，不取回本金，得零）；(4) 抽水從已結算的底池中按實際結算金額比例扣除；(5) 此規則已納入 Decision Log D13。
