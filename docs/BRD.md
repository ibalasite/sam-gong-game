...
> **核心目標宣言（F1）：** 在 **4 個月內**上線一款 Server-authoritative 即時多人三公遊戲，以「絕對公平」為差異化核心，於 Launch+6M 達成 Peak CCU ≥ 500、DAU ≥ 2,000、公平性 NPS ≥ 70，並於 Launch+9M 建立合規虛擬籌碼變現模式，付費率 ≥ 3%，奠定 Sayyo Games 華人休閒遊戲平台基礎。
...
| O3 | 建立變現模式（虛擬籌碼商店）**【F2條件】** 依據法律意見書結果：若IAP合法，啟用虛擬籌碼IAP；若存在法律疑慮，降級為廣告觀看換籌碼模式（Rewarded Ad），IAP停用。 | ARPPU ≥ USD 10，付費率 ≥ 3%（IAP模式）；DAU廣告點擊率 ≥ 15%（廣告模式） | 9 個月（法律意見書 2026-05-15 前決定） | Should |
...
| Server-authoritative 洗牌與判牌 | Must Have | O1 | 1. Server 洗牌亂度測試通過（卡方分佈 p>0.05）；2. Client 端無任何判牌邏輯代碼 |
| Room State + 玩家狀態同步 | Must Have | O1 | 1. 玩家狀態延遲 ≤ 100ms（P95）；2. 斷線重連後狀態完整恢復，無資料遺失 |
| 下注系統（虛擬籌碼）| Must Have | O1 | 1. 籌碼計算無浮點誤差（整數運算）；2. 籌碼總量守恆（發牌前後總和一致） |
| 大廳 + Matchmaking（快速配對 / 指定房間）| Must Have | O2 | 1. 等待時間中位數 ≤ 30 秒（500 CCU 壓測環境）；2. 配對失敗率 ≤ 5% |
| 像素風 / 賭場風 UI（至少一套完整）| Must Have | O1 | 1. Beta 封測視覺滿意度評分 ≥ 4/5；2. 移動端（375px 寬）無元素重疊或截斷 |
...
| 虛擬籌碼商店（IAP）**【F2條件：依法律意見書啟用】** | Should Have | O3 | 1. IAP 交易成功率 ≥ 99%；2. 支付失敗退款流程 ≤ 24 小時；（若降級廣告模式：廣告播放完成率 ≥ 80%） |
...
| 設計項目 | 約束值 | 說明 |
|---------|--------|------|
| **初始籌碼量** | 新玩家註冊贈送 10,000 虛擬籌碼 | 可完成至少 20 局基礎下注（500/局） |
| **每日免費贈送上限** | 5,000 籌碼/日（帳號登入觸發） | 防止通膨；超過 7 日未登入則累積不補發 |
| **等級門檻籌碼數（最低持有）** | 青銅：0；白銀：50,000；黃金：200,000；鉑金：500,000；鑽石：1,000,000 | 等級決定可加入的房間級距 |
| **每局最低下注** | 500 籌碼 | 限制低籌碼玩家進入高級房 |
| **每局最高下注上限（依等級）** | 青銅：5,000；黃金：50,000；鑽石：500,000 | 防止籌碼快速集中 |
| **零餘額救濟機制** | 餘額 < 500 時，系統自動補發 1,000 救濟籌碼（每帳號每日上限 1 次） | 防止用戶因零餘額流失，維持基本遊玩體驗 |
| **籌碼回收機制** | 房間系統抽水 5%（底池進入遊戲維運基金） | 控制籌碼通膨速率 |
...
|---------|------|
| **牌數** | 每人發 3 張牌 |
| **點數計算** | 3 張牌點數相加 mod 10；A=1, 2-9 面值, J/Q/K=0（10點牌） |
| **三公定義** | 三張牌均為 10 點牌（J/Q/K/10），點數合計為 30（mod 10 = 0），三公為最大牌型 |
| **比牌順序** | 三公 > 9 點 > 8 點 > ... > 1 點 > 0 點（非三公） |
| **同點比大小** | 同點數時比最大單張花色（黑桃 > 紅心 > 方塊 > 梅花）或最大單張點數（視台灣地區習慣） |
...
| 牌型 | 賠率 | 說明 |
|------|------|------|
| 三公（30點/三公牌型） | 1:3 | 莊家/其他玩家賠 3 倍下注 |
| 9 點 | 1:2 | 賠 2 倍下注 |
| 8 點以下 | 1:1 | 賠等額下注 |
| 平手（同點同花色） | 退注 | 退回下注籌碼，不計輸贏 |
...
| NFR-05 | **安全：資料加密** | 靜態敏感資料（密碼、KYC、支付） | AES-256 加密儲存 | 安全審查 + 滲透測試 | Must |
...
| 未成年保護（F7年齡驗證）| **v1.0年齡驗證方案：出生年份自填 + 手機 OTP 雙重驗證**。流程：①玩家填入出生年份 → ②系統驗算是否 ≥18 歲 → ③若通過，發送手機 OTP 驗證（6 碼，5 分鐘有效）→ ④OTP 驗證成功後帳號啟用。未完成驗證帳號只可進入教學模式，無法參與正式對戰。 | 帳號系統新增年齡欄位 + SMS OTP 服務整合（如 Twilio / AWS SNS） | 開發成本 + SMS 費用約 NT$0.3/次 |
| 平台審查 | 強調「娛樂性質、無實質獎勵」 | 文案與行銷材料 | 開發內建 |
| 防沉迷機制（F8）| **具體實施方案：** (1) 連續遊玩計時器：每局結束後累計，滿 2 小時（120分鐘）強制顯示休息提醒彈窗，玩家需主動確認後方可繼續；(2) 每日遊玩時間顯示：帳號主頁顯示今日累計遊玩時間；(3) 未成年帳號強制限制：年齡 <18 歲帳號每日遊玩時數上限 2 小時，達上限後強制登出，次日 0:00 重置；(4) 防沉迷提醒內容：「您已連續遊玩 X 分鐘，請適度休息，注意健康。」 | 帳號系統新增遊玩計時模組；後端 Session 時間追蹤 | 開發內建 |
...
| KYC / 實名制 | Legal + Ops | Eng（帳號模組）| 串接第三方 KYC 服務或手機 OTP 驗證方案 | 100% 新帳號經過年齡 + 身份驗證 | Alpha 前 |
...
| 聊天室內容合規（F15）| Product + Ops | Eng | 關鍵字過濾清單、舉報機制、訊息留存 30 日 | 過濾覆蓋率 ≥ 95%；舉報處理 SLA ≤ 24 小時 | 聊天室功能上線前 |
...
| Cocos Creator | `3.x`（major locked） | 鎖定大版本，patch 更新需 PoC 驗證 | 跨大版本升級需 EDD 評估 |
...
| **虛擬籌碼等級清單（F16）** | 依持有籌碼量劃分的帳號等級，決定可進入的房間級距：青銅（0+）、白銀（50,000+）、黃金（200,000+）、鉑金（500,000+）、鑽石（1,000,000+）。等級僅影響可用房間，不影響比牌規則或賠率 |
