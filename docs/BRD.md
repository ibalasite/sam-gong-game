...
| 下注系統（虛擬籌碼）| Must Have | O1 | 1. 籌碼計算無浮點誤差（整數運算）；2. **籌碼守恆 AC（F5/F16）：每局結算後，所有玩家淨增減籌碼之和 = 負（抽水額）；抽水 = floor(所有閒家本局下注加總（不含莊家本金）× 5%)，精確整數運算，誤差容忍 = 0** |
...
| 新手引導教學（Tutorial Onboarding）| Must Have | O2 | AC：首次登入玩家的教學完成率 ≥ 60%（Launch+1M量測，分母為當月首次登入帳號數）；教學包含：三公規則說明、第一局模擬牌局（不消耗籌碼）、籌碼系統說明；完成後解鎖正式對戰入口；**模擬牌局邏輯由 Server 執行（tutorial_mode=true，籌碼扣除邏輯跳過）；Client 僅呈現動畫，與正式牌局相同 Server-authoritative 路徑** |
...
| 設計項目 | 約束值 | 說明 |
|---------|--------|------|
| **初始籌碼量** | 新玩家註冊贈送 100,000 虛擬籌碼 | 可完成至少 200 局基礎下注（500/局），並可直接進入黃金廳 |
| **每日免費贈送上限** | 5,000 籌碼/日（帳號登入觸發）（每日 00:00 UTC+8 重置；超過 7 日未登入者不補發累積天數） | 防止通膨；超過 7 日未登入則累積不補發 |
...
| **每局最低下注** | 500 籌碼 | 所有等級房間的統一最低下注門檻（500 籌碼）；進入高級房間的主要門禁為等級持有量條件（見等級籌碼門檻），而非下注下限。 |
| **每局最高下注上限（依等級）** | 青銅：5,000；**白銀：15,000**；黃金：50,000；**鉑金：150,000**；鑽石：500,000（F12） | 防止籌碼快速集中 |
...
**玩家操作計時器**：每位閒家有 30 秒決定 Call 或 Fold（莊家下注後計時開始）；超時自動視為 Fold，已下注額留存底池；計時器倒數顯示於 Client UI。（30 秒為預設值，PRD 可調整；記入 Decision Log D11。）
...
2. 莊家查看手牌 → 下注底注（≥ 500 籌碼，≤ 當局最高限注）；**莊家操作計時器**：莊家有 30 秒完成底注下注；超時後系統自動以最低底注（500 籌碼）代為下注並進入Step 3。（規則與閒家計時器一致；記入 Decision Log D12）
...
6. **結算與籌碼分配（三步驟）**：
   - Step 6a（收齊）：Server 收齊所有閒家本局下注額，加入底池（莊家本金不入底池，莊家為淨支付方或淨收款方）；
   - Step 6b（抽水）：從閒家下注總額中扣除 5% 抽水（floor 取整，最少 1 籌碼）；
...
| 8 點以下 | 1:1 | 閒家勝：莊家向該閒家支付 下注額 × 1；莊家勝：閒家下注額歸底池 |
| 平手（同點同花色） | 退注 | 退回下注籌碼，不計輸贏 |

**賠率計算語義（Payout Semantics）**：
- 閒家勝時：閒家從底池取回自身下注額（1×），另外由莊家額外支付對應倍數賠率（N×）；總取回 = (1+N)× 下注額。
- 例：閒家下注 1,000 籌碼、三公勝（1:3）→ 取回 1,000（本金）+ 3,000（莊家賠付）= 共 4,000 籌碼；淨利潤 +3,000。
- 莊家勝時：閒家下注額全額歸底池，莊家從底池獲得（扣抽水後）。
- 抽水：每局底池總額（所有閒家下注加總，不含莊家本金）× 5%，floor取整，結算前扣除。
- 【抽水說明】：抽水僅從「莊家獲勝局的閒家下注額」中扣除，閒家獲勝局的本金與賠率不含抽水（由莊家全額支付）。

**結算方向（雙向）**：莊家與每位未 Fold 閒家進行獨立比牌結算。閒家勝→莊家支付對應賠率籌碼給該閒家；莊家勝→閒家下注額歸入底池。所有下注總額構成底池，結算前先扣除 5% 抽水（floor 取整），再依比牌結果分配剩餘籌碼。

**混合結果結算順序**（莊家同局有贏有輸時）：Server 採原子性三步驟：(1) 先將所有閒家（含贏家與輸家）下注額全部收入結算暫存池（只含輸家閒家下注額用於扣抽水；贏家閒家本金由莊家直接返還）；(2) 扣除 5% 抽水；(3) 依比牌結果依順時鐘順序逐一支付：贏家閒家從底池取回本金，莊家另行支付 N× 賠率；輸家閒家不取回；莊家最終淨增減 = 所有輸家下注總額 – 所有贏家賠率支出（含莊家破產規則 D13）。

**莊家結算破產處理**：若莊家在結算過程中籌碼不足以支付所有贏家賠率，採用以下規則：(1) 依閒家**順時鐘**順序逐一結算；(2) 每位贏家依序收取其應得賠率（先到先得）；(3) 莊家籌碼耗盡後，後續贏家僅能收取剩餘可用籌碼（等比例縮減至餘額耗盡）；(4) 抽水從已結算的底池中按實際結算金額比例扣除；(5) 此規則已納入 Decision Log D13。
...
| 防沉迷機制（F8）| **具體實施方案：** (1) 連續遊玩計時器：每局結束後累計，滿 2 小時（120分鐘）強制顯示休息提醒彈窗，玩家需主動確認後方可繼續；(2) 每日遊玩時間顯示：帳號主頁顯示今日累計遊玩時間；(3) 未成年帳號強制限制：年齡 <18 歲帳號每日遊玩時數上限 2 小時，達上限後強制登出，次日 0:00 重置；(4) 防沉迷提醒內容：「您已連續遊玩 X 分鐘，請適度休息，注意健康。」**【F18 計時器重置條件】** (5) 連續遊玩計時器重置條件：玩家主動登出後重置；或離線（網路斷線 / App 關閉）超過 **30 分鐘**後重置；App 切換至背景超過 30 分鐘視為中斷並重置計時器 | 帳號系統新增遊玩計時模組；後端 Session 時間追蹤 | 開發內建 |
...
| 未成年保護（防沉迷）| Product | Eng + Legal | 年齡驗證閘 + 連續遊玩提醒 + 未成年時數限制 | QA 驗收：連續 2 小時提醒出現率 100%；未成年帳號 2 小時強制登出 | 2026-07-21（Beta前）|
...
timeline
    ...
    section Growth（4–12個月）
        ...
        Launch + 6M : 每日任務完整版、排行榜優化（VIP訂閱列入v1.x計畫，Out of Scope v1.0）（2027-02-21）
...
| D7 | PENDING（決策截止 2026-07-21） | 美術風格最終選定 | **暫定：像素風為主；2026-07-21 前由 PM + Art Director 依 A/B 測試數據決定** | 像素風成本低風險小，賭場風僅試做關鍵畫面，Beta 數據驅動決策 | PM + Art Director | §5.1、美術資源分配 |
...
| D12 | 2026-04-22 | 莊家自動下注行為（超時） | 莊家有30秒完成底注下注；超時後系統自動以最低底注（500籌碼）代為下注並進入Step 3；規則與閒家計時器一致 | 防止莊家長時間不下注導致牌局卡死；與閒家超時規則對稱，確保遊戲流程完整性 | PM + Game Designer | §5.5 牌局流程 Step 2 |
| D13 | 2026-04-22 | 莊家結算破產處理順序 | 採順時鐘優先支付，餘額耗盡後按剩餘可用比例縮減 | 確保Server邏輯確定性，防止爭議 | PM + Engineering Lead | §5.5 結算邏輯 |
...
| Rake / 抽水 | 每局牌局系統從底池中扣取的手續費，本專案固定為底池總額的5%（floor取整）。 |
