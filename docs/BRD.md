...
| **文件版本** | v0.11-draft |
| **狀態** | DRAFT（BRD Fixer Round 10 完成）|
...
| 下注系統（虛擬籌碼）| Must Have | O1 | 1. 籌碼計算無浮點誤差（整數運算）；2. **籌碼守恆 AC（F5/F16）：每局結算後，所有玩家淨增減籌碼之和 = 負（抽水額）；抽水 = floor(本局莊家獲勝之閒家下注額加總 × 5%，floor取整，最少1籌碼）；閒家獲勝局不計入抽水底數；每局結算後，玩家籌碼增減：贏家閒家淨增 N× 下注額；莊家淨增 = 所有輸家下注額 - 所有贏家賠率 - 抽水；抽水 = floor(輸家閒家下注額加總 × 5%)，精確整數運算，誤差容忍 = 0** |
...
**籌碼 500-999 邊緣情況**：若玩家餘額介於 500-999 籌碼（高於救濟觸發值 <500，但低於最低房間入場門檻青銅廳 ≥1,000），系統於大廳顯示提示：『您的籌碼不足進入任何房間，請完成每日任務或等待每日免費籌碼（每日 00:00 UTC+8 重置）』；不觸發救濟機制；玩家需透過每日贈送（+5,000）後恢復進房資格。
...
6. **結算與籌碼分配（三步驟）**：
   - Step 6a（確認結果與底池）：Server 確認每位閒家的比牌結果；莊家獲勝之閒家下注額及棄牌（Fold）閒家下注額歸入底池；莊家敗之閒家下注額不入底池（由莊家直接支付本金 1× + 賠率 N×）。
...
**混合結果結算順序**（莊家同局有贏有輸時）：Server 採原子性三步驟：(1) 確認每位閒家的本局比牌結果（贏/輸/Fold）；莊家敗的閒家下注額由莊家從自身籌碼支付（本金+賠率）；莊家勝的閒家下注額歸入底池；(2) 從底池（輸家閒家下注額合計）扣除 5% 抽水（floor取整，最少1籌碼）；(3) 依比牌結果依順時鐘順序逐一支付：贏家閒家由莊家直接取回本金（1×下注額）+ N×賠率（均由莊家支付）；輸家閒家不取回；莊家最終淨增減 = 所有輸家下注總額 – 所有贏家賠率支出 – 抽水（含莊家破產規則 D13）。
...
| Fold／棄牌 | 閒家在莊家下注底注後，選擇放棄本局比牌；已入底池之下注額（若有）不退還；手牌保持暗牌，結算時不公開（v1.0規則，見§5.5）。 |
