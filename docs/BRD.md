...
| O2 | 驗證玩家留存率 | Beta：7 日留存率 ≥ 30% | 6 個月 | Should |
...
| 術語 | 業務定義 |
|------|---------|
| 三公（Sam Gong）| 中式三張牌撲克，以牌面點數之和個位數比大小，9 最大（公牌）|
| 公牌 | 三張牌點數和個位數為 0（10 的倍數），為最大牌型 |
| Server Authoritative | 伺服器持有並計算所有遊戲狀態，客戶端只做呈現 |
| Colyseus Schema | Colyseus 提供的可差分同步的狀態結構定義工具 |
| Room（房間）| Colyseus 的多人遊戲房間單位，容納 2-6 位玩家 |
| 結算（Settlement）| 一局牌局結束後，籌碼依勝負重新分配的過程 |
| 斷線重連（Reconnect）| 玩家網路中斷後在時間窗口內重新連入房間的機制 |
