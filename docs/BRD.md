...
| **VIP 訂閱系統（F13）** | **Won't Have（v1.0）**（Out of Scope，§13 REQ-021 為 Out of Scope v1.0） | O3 | N/A（v1.0 不實作；功能描述：VIP月費 USD 9.99；權益包含每日額外籌碼獎勵 +50%、VIP 頭像框、優先配對；列入 v1.x Backlog） |
...
| **同點比大小（F3 確定規則）** | 同點數時：**第一順位比最大單張花色**（黑桃 > 紅心 > 方塊 > 梅花）；花色亦同時比**最大單張點數**（A > K > Q > J > 10 > 9 > ... > 2）。兩者皆相同時判平手退注。不採用「視地區習慣」之模糊規則（決策見 D8） |
...
| 未成年保護（F7年齡驗證）| **v1.0年齡驗證方案：出生年份自填 + 手機 OTP 雙重驗證**。流程：①玩家填入出生年份 → ②系統驗算是否 ≥18 歲 → ③若通過，發送手機 OTP 驗證（6 碼，5 分鐘有效）→ ④OTP 驗證成功後帳號啟用。未完成驗證帳號只可進入教學模式，無法參與正式對戰。**【F11 風險備注】OTP 限制：OTP 只驗證手機持有人，無法確認實際年齡。v1.0 門檻為出生年自填 + OTP。若法律意見書（2026-05-15）認定此方案不足，升級至第三方 KYC（TW 自然人憑證）。升級觸發條件見 §9.4 合規矩陣。** | 帳號系統新增年齡欄位 + SMS OTP 服務整合（如 Twilio / AWS SNS） | 開發成本 + SMS 費用約 NT$0.3/次 |
...
| 依賴項 | 類型 | 負責方 | 若延誤的影響 |
|--------|------|--------|------------|
| Cocos Creator 3.8.x（minor locked）（F16） | 技術 | Cocos 官方 | 開發阻塞 |
| Colyseus 0.15.x（locked）（F25） | 技術 | Colyseus 官方 | 開發阻塞 |
| **PostgreSQL 16.x（locked）**（F10） | 技術 | 內部 DBA / 雲服務 | 持久層開發阻塞 |
| **Redis 7.x（locked）**（F10） | 技術 | 內部 SRE / 雲服務 | Matchmaking / Session Cache 阻塞 |
